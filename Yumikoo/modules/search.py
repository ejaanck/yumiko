...
@Yumikoo.on_message(filters.command('google'))
async def search_(Yumikoo: Yumikoo, msg: Message):
...
@Yumikoo.on_message(filters.command('stack'))
async def stack_search_(Yumikoo: Yumikoo, msg: Message):
    split = msg.text.split(None, 1)
    if len(split) == 1:
        return await msg.reply_text("**ɢɪᴠᴇ ǫᴜᴇʀʏ ᴛᴏ sᴇᴀʀᴄʜ**")
    to_del = await msg.reply_text("**sᴇᴀʀᴄʜɪɴɢ ᴏɴ ɢᴏᴏɢʟᴇ...**")

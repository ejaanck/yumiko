...
@Yumikoo.on_message(filters.command("ban", COMMAND_HANDLER) & admin_filter)
async def ban(_, message):
    ...
    if message.chat.type == enums.ChatType.PRIVATE:
        ...
        try:
            user = await Yumikoo.get_user(message.command[1])
            admin_check = await Yumikoo.get_chat_member(chat_id, user_id)
            if admin_check.privileges.can_restrict_members:
                await message.chat.ban_member(user.id)
                await msg.edit_text(f"<code>❕</code><b>ʙᴀɴ ᴇᴠᴇɴᴛ</b>\n <b>•  ʙᴀɴɴᴇᴅ ʙʏ:</b> {message.from_user.mention}\n <b>•  ᴜsᴇʀ:</b> [{user.first_name}](tg://user?id={user.id})", reply_markup=button)      
...
@Yumikoo.on_message(filters.command("unban", COMMAND_HANDLER) & admin_filter)
...
@Yumikoo.on_message(filters.command("mute", COMMAND_HANDLER) & admin_filter)
...
@Yumikoo.on_message(filters.command("unmute", COMMAND_HANDLER) & admin_filter)
...
@Yumikoo.on_message(filters.command("kick", COMMAND_HANDLER) & admin_filter)
...
@Yumikoo.on_message(filters.command("halfpromote") & admin_filter)
